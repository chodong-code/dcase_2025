...
class PaSST(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # === 반드시 채널을 1로 맞춰주세요 ===
        if x.shape[1] != 1:
            if first_RUN: print(f"[WARNING] patch_embed 입력 채널이 1이 아님. 강제 변환합니다. {x.shape}")
            x = x[:, :1, :, :]  # 또는 x = x.mean(dim=1, keepdim=True)
        if first_RUN: print("before patch_embed", x.size())
        #---------------------------------------------------------------추가----------------------------------------------------------
